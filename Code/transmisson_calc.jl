...
## parameters (calculation for perpendicular incident) (https://refractiveindex.info/)
lambda = 520 # nm
N = [1.0; 3.812 + 0.1903im; 4.1891+0.3633im] # air - window layer - absorber layer
num_layer = 3 # air - window layer - absorber layer
D = 100 # thickness of window layer
## function for fresnel coefficients
...
## function for matrix for interface propagation
function interface_prop(n,m,pol)
    r,t = fresnel_coeff(n,m,pol)
    I = 1/t * [1 r;r 1]
    return I
end
## function for matrix for propagation through film
function film_prop(d,n)
    beta = 2*pi*d*n/lambda
