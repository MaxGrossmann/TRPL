...
cd(string(path_to_trpl,"\\Optimizations\\Syn\\","syn_data_",idx_syn_data))
...
cd(string(path_to_trpl,"\\Data\\Syn\\Params"))
...
if out 
	...
	xlsx_path = string(path_to_trpl,"\\Results\\Syn",file_name)
	## save output
	if isfile(xlsx_path)
		rm(xlsx_path)
	end
